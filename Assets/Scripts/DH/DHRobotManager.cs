...
public class DHRobotManager : MonoBehaviour {
	...
	void CreateRobot () {
		...
		float firstXRotation = PlayerPrefs.GetFloat ("FirstXRotation");
		float firstYRotation = PlayerPrefs.GetFloat ("FirstYRotation");
		float firstZRotation = PlayerPrefs.GetFloat ("FirstZRotation");
	...
	void Set_DHMatrix () {
		...
		for (int i = 0; i < nWorlds_; i++) {
			...
			string A = PlayerPrefs.GetString ("world" + i.ToString () + "a");
			variableMatrix[i,0] = Regex.Match (A, "^[-+]?[0-9]*\\.?[0-9]*[+-]?v$").Success;

			string D = PlayerPrefs.GetString ("world" + i.ToString () + "d");
			variableMatrix[i,1] = Regex.Match (D, "^[-+]?[0-9]*\\.?[0-9]*[+-]?v$").Success;

			string AL = PlayerPrefs.GetString ("world" + i.ToString () + "al");
			variableMatrix[i,2] = Regex.Match (AL, "^[-+]?[0-9]*\\.?[0-9]*[+-]?v$").Success;

			string TH = PlayerPrefs.GetString ("world" + i.ToString () + "th");
			variableMatrix[i,3] = Regex.Match (TH, "^[-+]?[0-9]*\\.?[0-9]*[+-]?v$").Success;
		}
	}
