...
public class PrefabJoint1 : MonoBehaviour {
	//****** CONSTATS *******
	const bool X =  true;
	const bool Z =  false;
	...
	void Update () {
		//SET TEXT ANGLE
		//GetComponent<TextMesh>().text = "  " + this.transform.FindChild ("visibleObject").localEulerAngles.z.ToString ("F2") + "º";
		rotateChildrens (a);
	...
	public void rotateOverZ(bool sign = true){
		rotationAxis = Z;
		rotationSign = sign;
		if(sign == true)
	...
	public void rotateOverX(bool sign = true){
		rotationAxis = X;
		rotationSign = sign;
		if(sign == true)
